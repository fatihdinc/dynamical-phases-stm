...
def train_rnn_model(process_idx,delay_interval,learning_rate,sh=0,sv=1):
    ...
    import warnings
    warnings.filterwarnings('ignore')
    file_name =f'./results_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/class.txt'
    file_name2 =f'./results_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/psd_ratio.txt'
    
        
      # Load the image
    f = np.load(f'./results_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz')
    ...
    model.load_state_dict(torch.load(f'./results_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_final.pt', weights_only=True))
...
psds = np.zeros([lr_all.shape[0],21,n_exp]) + np.nan
...
plt.savefig('phase_diagram_post.pdf')
plt.show()
