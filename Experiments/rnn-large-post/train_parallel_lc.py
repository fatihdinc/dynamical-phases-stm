...
def train_rnn_model(process_idx,delay_interval,num_epochs,learning_rate,sh=0,sv=1):
    ...
    # Training Loop
    epoch_bar = tqdm(range(num_epochs),desc=f"Process {process_idx}")
    for epoch in epoch_bar:
        ...
        if (epoch) % 50 == 0:  # Adjust save frequency as needed
            if sv:
                os.makedirs(f'./checkpoints_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
                torch.save(model.state_dict(), f'./checkpoints_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_epoch_{epoch + 1}.pt')
                np.savez(f'./checkpoints_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                         loss_list=loss_list,
            ...
            if sh==1:
                plt.show()
            if sv == 1:
                plt.savefig(f'./checkpoints_post/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_epoch_cur.png')
            plt.close()

    # Save final model
    if sv == 1:
        os.makedirs(f'./results_post_longer_epoch/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
        torch.save(model.state_dict(), f'./results_post_longer_epoch/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_final.pt')
        np.savez(f'./results_post_longer_epoch/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                 loss_list=loss_list,
                 reaction_accuracy_list=reaction_accuracy_list,
                 reaction_reliability_list=reaction_reliability_list,
                 W_rec_grad_list=W_rec_grad_list)
    ...
    plt.tight_layout()
    if sv == 1:
        plt.savefig(f'./results_post_longer_epoch/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_final.png')
    plt.close()
...
for k in range(15):
    ...
    num_epochs = int( np.maximum(1e3,1e5 *1e-4 * 10 / learning_rate))
