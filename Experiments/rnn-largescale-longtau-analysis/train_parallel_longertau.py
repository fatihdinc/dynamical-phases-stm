...
def train_rnn_model(process_idx,delay_interval,num_epochs,learning_rate,sh=0,sv=1):
    ...
    # Save final model
    if sv == 1:
        os.makedirs(f'./results_longertau/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
        torch.save(model.state_dict(), f'./results_longertau/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_final.pt')
        np.savez(f'./results_longertau/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                 loss_list=loss_list,
    ...
    plt.tight_layout()
    if sv == 1:
        plt.savefig(f'./results_longertau/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_final.png')
    plt.close()
