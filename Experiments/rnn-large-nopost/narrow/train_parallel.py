...
def train_rnn_model(process_idx,delay_interval,num_epochs,learning_rate,sh=0,sv=1):
    ...
    for epoch in epoch_bar:
        ...
        if (epoch) % 50 == 0:  # Adjust save frequency as needed
            if sv:
                os.makedirs(f'./checkpoints_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
                torch.save(model.state_dict(), f'./checkpoints_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_epoch_{epoch + 1}.pt')
                np.savez(f'./checkpoints_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                         loss_list=loss_list,
            ...
            if sh==1:
                plt.show()
            if sv == 1:
                plt.savefig(f'./checkpoints_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_epoch_cur.png')
            plt.close()

    # Save final model
    if sv == 1:
        os.makedirs(f'./results_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
        torch.save(model.state_dict(), f'./results_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_final.pt')
        np.savez(f'./results_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                 loss_list=loss_list,
                 reaction_accuracy_list=reaction_accuracy_list,
                 reaction_reliability_list=reaction_reliability_list,
                 W_rec_grad_list=W_rec_grad_list)
    ...
    plt.tight_layout()
    if sv == 1:
        plt.savefig(f'./results_finetune/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_final.png')
    plt.close()

    return f"Process {process_idx} completed."


lr_all = np.logspace(-0.5,-2,20)[:15];
