...
def train_rnn(rnn, T, T_disc, total_time, learning_rate, num_epochs, plot_interval, rnn_type, factor):
    ...
    for epoch in pbar:
        ...
        if epoch % 1000 == 0:
            ...
            plt.subplot(141)
            plt.semilogy(np.array(loss_history)/loss_history[0])
            ...
            plt.subplot(144)
            sorted_indices = get_ranked_indices(abs(x_new.T))
            plt.imshow(x_new[sorted_indices,:3*total_time],aspect = 'auto')
            plt.colorbar()
...
T_disc = int(T)
total_time = T_disc +2
learning_rate = .001  # Do 0.01 and 0.005
num_epochs = 500000
plot_interval = 1
...
np.savez('low_T.npz',grad_history = grad_history,loss_history = loss_history)
