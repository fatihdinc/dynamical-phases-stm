...
def plot_results(original_rnn,r,epoch,sv=1):
    ...
    if sv == 1:
        plt.savefig(f'plots/lr_{learning_rate}/cur_epoch{epoch}.pdf')
        plt.close()
...
def train_rnn(rnn, T, T_disc, total_time, learning_rate, num_epochs, plot_interval, rnn_type, factor):
    ...
    if os.path.exists(f'plots/lr_{learning_rate}'):
        shutil.rmtree(f'plots/lr_{learning_rate}')
        
    
    os.makedirs(f'./plots/lr_{learning_rate}', exist_ok=True)
    ...
    for epoch in pbar:
        ...
        if loss.item() /loss_history[0]  < 1e-4:
            break
        ...
        if epoch % 1000 == 0:
            ...
            plt.axvline(T)
            plt.colorbar()
            plt.savefig(f'plots/lr_{learning_rate}/cur_epoch{epoch}_activity.pdf')
            plt.show()
...
T_disc = int(T)
total_time = T_disc +2
learning_rate = .0001  # Do 0.01 and 0.005
num_epochs = 5000000
plot_interval = 1
current_time = datetime.now().time()

print('Start time:',current_time.strftime("%H:%M:%S"), learning_rate)
...
np.savez(f'results_lr_{learning_rate}.npz',grad_history = grad_history,loss_history = loss_history)
