...
def train_rnn_model(process_idx,delay_interval,num_epochs,learning_rate,sh=0,sv=1):
    ...
    reaction_interval = delay_interval
    ...
    for epoch in epoch_bar:
        for _, (inp, gt_out) in enumerate(dataloader):
            ...
            [gt_out_new,_] = torch.max(gt_out,1,keepdims = True);
            gt_out = np.maximum(gt_out_new,gt_out)
            

            
            

            weights = torch.zeros_like(gt_out).to(device)+1
            ...
            #    f"Process: {process_idx}. lr: {learning_rate}. T: {int(delay_interval * 1000)}. Epoch loss: {round(loss.item(), 4)} - Accuracy: {round(reaction_accuracy, 4)} - Reliability: {round(reaction_reliability, 4)}"
            #)
     
        if np.max(loss_list)>100:
            # Save final model
            if sv == 1:
               os.makedirs(f'./results_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
               torch.save(model.state_dict(), f'./results_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_final.pt')
               np.savez(f'./results_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                        loss_list=loss_list,
               ...
               plt.plot(loss_list)
               plt.title(f'Process{process_idx} failed.')
               plt.savefig(f'./results_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_final.png')
        ...
        # Save model and plots periodicallys
        if (epoch) % 50 == 0:  # Adjust save frequency as needed
            if sv:
                os.makedirs(f'./checkpoints_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}', exist_ok=True)
                torch.save(model.state_dict(), f'./checkpoints_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/model_epoch_{epoch + 1}.pt')
                np.savez(f'./checkpoints_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/training_details.npz',
                 loss_list=loss_list,
                 reaction_accuracy_list=reaction_accuracy_list,
                 reaction_reliability_list=reaction_reliability_list,
                 W_rec_grad_list=W_rec_grad_list)
            try:
                ...
                if sh==1:
                    plt.show()
                if sv == 1:
                    plt.savefig(f'./checkpoints_fixed/lr{learning_rate}_delay_{delay_interval}/process_{process_idx}/metrics_plot_epoch_cur.png')
                plt.close()
            except:
                plt.plot(loss_list)
                plt.title(f'Process{process_idx} failed.')
