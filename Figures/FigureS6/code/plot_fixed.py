...
for k in [0,1]:
    ...
    plt.errorbar(np.linspace(1,3000,3000),np.nanmean(temp,0),np.nanstd(temp,0)/np.sqrt(temp.shape[0]),color = colors[k])
...
met = np.zeros([lr_all.shape[0],20,2])
...
for k in range(T_all.shape[0]):
    ...
    plt.scatter(np.log(lr_all), (temp), label='Noisy Data', color=colors[st], alpha=0.6)

    popt, pcov = curve_fit(sigmoid, np.log(lr_all),
                        temp, p0=[-5,-10],maxfev = 1000000)  # Initial guess for a
    a_fit = popt[0]#+1/popt[1]
    #try:
    #    a_fit = T_all[np.where(temp>0.1)[0][0]] #popt[0]
    #except:
    #    a_fit = 0
    lr_new = np.linspace(np.log(lr_all).min(),np.log(lr_all).max(),1000)
    
    # Plot the data and the fitted function
    plt.plot(lr_new, (sigmoid(lr_new, *popt)), label=f'Fitted Function (a={1000*a_fit:.3f})', color=colors[st])
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.show()
    a_all[k] = np.exp(a_fit)
...
plt.scatter(T_all, a_all,20, color='black')
...
plt.title(f" Critical exponent: {b_fit:.4f} ± {b_err:.4f}, R2: {r2:.4f}, p {p_value:.4f}")
